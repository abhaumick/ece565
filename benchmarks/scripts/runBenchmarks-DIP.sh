...
for program in "${listOfBenchmarks[@]}"; do
    ...
    for throttleValue in "${bipThrottle[@]}"; do
        ...
        # BIP
   	    $gem5opt -d $folder/DIP-$throttleValue/$program $pathToRunPy $options $cacheOptions -b $program --bip_throttle=$throttleValue --standard-switch=$standardSwitch --maxinsts=$maxinsts --fast-forward=$fastFwdCount --warmup-insts=$warmupCount --at-instruction
